 ...
 * WebSocket host for multiplayer room management
 */

import * as Sim from './archimedes_sim.js';
...
export function createArchimedesHost() {
  ...
  // Fixed tick loop for all archimedes rooms (20 Hz for smooth physics)
  setInterval(() => {
    const now = nowMs();
    const toDelete = [];
    ...
    for (const id of toDelete) {
      rooms.delete(id);
      console.log(`[ws][archimedes] Room ${id} expired`);
    }
  }, 50); // 20 Hz
  ...
  function handleMessage(ws, msg) {
    switch (msg.type) {
      ...
      case 'input': {
        ...
        const room = rooms.get(ws.room);
        if (!room) return;
        Sim.applyInput(room.state, ws.playerId, msg, nowMs());
        break;
      }
      ...
      case 'pause': {
        ...
        const room = rooms.get(ws.room);
        if (!room) return;
        Sim.togglePause(room.state, ws.playerId);
        break;
      }
      ...
      case 'resume': {
        ...
        const room = rooms.get(ws.room);
        if (!room) return;
        Sim.resume(room.state, ws.playerId);
        break;
      }
    }
  }
