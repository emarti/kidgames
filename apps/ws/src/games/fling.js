import * as Sim from './fling_sim.js';
...
export function createFlingHost() {
  ...
  // Fixed tick loop for all fling rooms
  setInterval(() => {
    const now = nowMs();
    const toDelete = [];
    ...
    for (const id of toDelete) {
      rooms.delete(id);
      console.log(`[ws][fling] Room ${id} expired`);
    }
  }, TICK_MS);
  ...
  function handleMessage(ws, msg) {
    switch (msg.type) {
      ...
      case 'restart': {
        if (!ws.room) return;
        const room = rooms.get(ws.room);
        if (room) Sim.restart(room.state, nowMs());
        break;
      }
      ...
      case 'select_level': {
        if (!ws.room) return;
        const room = rooms.get(ws.room);
        if (room) Sim.setLevel(room.state, msg.level);
        break;
      }
      ...
      case 'next_level': {
        if (!ws.room) return;
        const room = rooms.get(ws.room);
        if (room) Sim.nextLevel(room.state);
        break;
      }
      ...
      case 'set_guides': {
        if (!ws.room) return;
        const room = rooms.get(ws.room);
        if (room) Sim.setGuides(room.state, msg.show);
        break;
      }
      ...
      case 'input': {
        ...
        if (msg.action === 'aim') {
          Sim.applyAim(room.state, ws.playerId, msg.angle, msg.power);
        } else if (msg.action === 'fire') {
          Sim.fire(room.state, ws.playerId, nowMs());
        }
        break;
      }
