import * as Sim from './alphabet_sim.js';
...
export function createAlphabetHost() {
  ...
  setInterval(() => {
    ...
    for (const id of toDelete) {
      rooms.delete(id);
      console.log(`[ws][alphabet] Room ${id} expired`);
    }
  }, TICK_MS);
  ...
  function handleMessage(ws, msg) {
    switch (msg.type) {
      ...
      case 'select_level': {
        if (!ws.room) return;
        const room = rooms.get(ws.room);
        if (room) Sim.selectLevel(room.state, msg.index);
        break;
      }
