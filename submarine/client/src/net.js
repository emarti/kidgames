export class Net extends EventTarget {
  ...
  connect() {
    ...
    const defaultProdUrl = `${protocol}://${window.location.host}/games/submarine/ws`;
    ...
    if (configuredUrlRaw) {
      ...
      try {
        const url = new URL(u);
        if (!/\/ws\/?$/.test(url.pathname)) url.pathname = '/games/submarine/ws';
        wsUrl = url.toString();
      } catch {
        wsUrl = u;
      }
    ...
    this.socket.onopen = () => {
      this.connected = true;
      this.ready = false;
      this.socket.send(JSON.stringify({ type: 'hello', gameId: 'submarine', protocol: 1 }));
    ...
    this.socket.onmessage = (event) => {
      try {
        ...
        console.error('Net parse error', error);
      }
    };
  ...
  send(type, payload = {}) {
    ...
    try {
      ...
      // ignore send races around reconnect/close
    }
  }
