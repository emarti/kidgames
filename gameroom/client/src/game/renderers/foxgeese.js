/**
 * foxgeese.js — Phaser 3 renderer for Fox and Geese.
 * Fox = 'black' (amber), Geese = 'white' (white circles).
 *
 * Renderer interface:
...
// ─── Topology (self-contained, matches foxgeese_sim.js exactly) ──────────────
...
function _foxRadius()   { return Math.max(14, Math.round(_bs * 0.06)); }
function _geeseRadius() { return Math.max(11, Math.round(_bs * 0.05)); }
...
function _drawBoard() {
  ...
  // All edges (draw each pair only once: i < j).
...
function _drawLastMoveMarker(lastMove) {
  ...
  const col  = lastMove.color === 'black' ? 0xff6600 : 0xcccccc;
...
function _drawPieces(board, selected, validDests, pendingJump) {
  ...
  const fr = _foxRadius();
  const gr = _geeseRadius();

  for (let i = 0; i < 33; i++) {
    const color = board[i];
    if (!color) continue;
    const p   = _px(i);
    const r   = color === 'black' ? fr : gr;
    ...
    // Pending-jump ring (gold, more prominent — tells player they must continue).
    if (i === pendingJump) {
  ...
  // Valid destination highlights (on empty squares).
  if (validDests) {
    const hr = _geeseRadius();
...
function _drawPiece(gfx, px, py, r, color, alpha = 1) {
  if (color === 'black') {
    // Fox: amber/orange.
    gfx.fillStyle(0xe07020, alpha);
    gfx.fillCircle(px, py, r);
    gfx.lineStyle(2, 0x8b4000, alpha);
    gfx.strokeCircle(px, py, r);
    // Highlight glint.
    gfx.fillStyle(0xffaa44, 0.4 * alpha);
...
/** Compute legal destination indices for the piece at fromIdx (local helper). */
function _computeLegalDests(gameState, fromIdx, color) {
  ...
  if (color === 'black') {
    ...
    if (caps.length > 0 || pendingJump !== null) return caps;
    // No captures: adjacent empty squares.
...
function _redraw() {
  if (_lastGameState && _lastCtx) foxgeeseRenderer.draw(_lastGameState, _lastCtx);
}
...
function _updateDrag(px, py) {
  ...
  _dragGfx.clear();
  const r = _dragColor === 'black' ? _foxRadius() : _geeseRadius();
...
const foxgeeseRenderer = {
  ...
  draw(gameState, ctx) {
    ...
    // Compute valid destinations for selected piece or pending-jump fox.
    let validDests    = null;
  ...
  formatTurnText(gameState) {
    if (gameState.gameOver) {
      return gameState.winner === 'black' ? '🦊 FOX wins!' : '🪿 GEESE win!';
    }
    if (gameState.pendingJump !== null) {
      return '🦊 FOX — continue jump!';
    }
    const who = gameState.turn === 'black' ? '🦊 FOX' : '🪿 GEESE';
    return `${who} to move`;
  },

  formatTurnColor(gameState) {
    if (gameState.gameOver) return '#ffd700';
    return gameState.turn === 'black' ? '#ff8800' : '#ffffff';
  },
  ...
  getGameOverInfo(gameState) {
    ...
    const foxWon = gameState.winner === 'black';
    return {
      title:  'GAME OVER',
...
export default foxgeeseRenderer;
